...
echo "============================================="
echo " DWAARPAL BARE-METAL SCALING BENCHMARK "
echo "============================================="
...
echo "Starting clean Redis environment in Docker..."
...
docker-compose -f deployments/docker-compose.standalone.yml up redis -d >&2
...
for N in "${REPLICAS[@]}"; do
    ...
    for (( i=1; i<=N; i++ )); do
        PORT=$((8080 + i))
        PORT=$PORT GRPC_PORT=$((50050 + i)) REDIS_URL="redis://localhost:6379" ./dwaarpal-api >/dev/null 2>&1 &
        PIDS+=($!)
    done
    ...
    NUM_NODES=$N k6 run --summary-export=benchmark/native_results_${N}.json benchmark/k6_native_load_test.js >&2 || true

    # Extract base throughput if N=1
    if [ "$N" -eq 1 ]; then
        BASE_THROUGHPUT=$(python3 -c "import json; print(json.load(open('benchmark/native_results_1.json'))['metrics']['http_reqs']['rate'])")
    fi
done
...
try:
    with open(f'benchmark/native_results_{N}.json') as f:
        data = json.load(f)
